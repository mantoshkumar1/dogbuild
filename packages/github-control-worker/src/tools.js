...
export const TOOLS = [
  ...
  { name: "update_control_comment", profiles: R, description: "Rewrite one CONFIGURED control comment (a status slot) in place. The target must appear in the Worker's CONTROL_COMMENTS configuration — it cannot be chosen by the caller. expected_updated_at is mandatory, the body is bounded, and only metadata is returned. This is the browser-free replacement for editing a status comment by hand.", inputSchema: obj({ ...repoParams, comment_id: { type: "number", description: "Must be a configured control comment id" }, body: { type: "string", description: "Replacement body (bounded)" }, expected_updated_at: { type: "string", description: "REQUIRED. The comment's updated_at as last read; a stale value is refused before any write." } }, ["owner", "repo", "comment_id", "body", "expected_updated_at"]) },
